...
promedio_salario_por_lenguaje = dataset.groupby('Lenguajes_Programacion_Limpio')['Salario'].mean()
...
plt.title('Salario promedio de los grupos de lenguajes de programación')
...
for bar in bars:
    yval = bar.get_height()
    plt.text(bar.get_x() + bar.get_width()/2, yval, round(yval, 2), ha='center', va='bottom', size=8)
